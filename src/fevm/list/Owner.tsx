...
import { nft_owner_columns, token_owner_columns } from '@/contents/contract'
...
export default observer(
  ({ type, id }: { type: string; id?: string | string[] }) => {
    ...
    const load = async (cur?: number, filter?: string) => {
      ...
      const axiosUrl =
        type === 'nfts' ? apiUrl.contract_NFTOwners : apiUrl.contract_ERC20Owner
    ...
    const columns = useMemo(() => {
      const newColumns =
        type === 'nfts' ? nft_owner_columns : token_owner_columns
