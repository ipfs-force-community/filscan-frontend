...
export default () => {
  ...
  const renderChildren = (item: any) => {
    ...
    const options =
      dataIndex === 'compile_version' ? versionOptions : licensesOptions
    let content
    switch (item.type) {
      case 'Input':
        content = (
          <Input className="custom_input !h-9" placeholder={tr(placeholder)} />
        )
        break
      default:
        content = (
  ...
  return (
    <>
      ...
      <BrowserView>
        <div>
          <span className="mx-2.5 flex flex-col gap-y-2.5 text-xl font-medium">
            {tr('verify_title')}
          </span>
          <span className="text_des mx-2.5  text-xs">{tr('verify_des')}</span>
        </div>
        <div className="card_shadow border_color mt-2.5 flex flex-col items-center  rounded-xl border p-5">
          <div className="text_des text-xs leading-6">
            {tr('verify_content')}
          </div>
          <div className="mt-12 w-[500px]">
            <Form
              className="custom_form !w-full"
              form={form}
              size="small"
            ...
            >
              ...
              <Form.Item>
                <Space>
                  <Button
                    htmlType="submit"
                    className="primary_btn !h-9 !w-[140px]"
                  >
                    {tr('next')}
                  </Button>
                  <Button
                    htmlType="reset"
                    className="cancel_btn !h-9 !w-[140px]"
                  >
                    {tr('reset')}
                  </Button>
                </Space>
