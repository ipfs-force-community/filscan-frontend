...
export default ({
  ...
  fileData: any
  confiles: any
  onChange: (file: any, type: string) => void
}) => {
  ...
  return (
    <>
      <div>
        ...
        >
          <span className=" flex h-8 items-center gap-x-2">
            <Button className="primary_btn flex h-8 items-center gap-x-2">
              {getSvgIcon('uploadIcon')}
              {tr(type === 'standard' ? 'file_name_json' : 'file_name')}
            </Button>
            <span className="text_des text-xs">
              {type === 'standard' ? tr('file_name_json') : tr('file_name')}
            </span>
          </span>
        </Upload>

        {fileData &&
          Object.keys(fileData)?.map((acekey: string, index: number) => {
            const aceItem = fileData[acekey]
            return (
              <div key={index} className="my-2.5">
                <div className="mb-2.5 flex items-center justify-between">
                  <span className="text_des flex items-center gap-x-2">
                    {getSvgIcon('fileIcon')}
                    {aceItem.name}
                  </span>
                  <span
        ...
        {type === 'standard' && (
          <ul className="border_color mt-5 rounded-[5px] border p-5 leading-6">
            {verify.meta_list_des.map((listItem: any, index: number) => {
              return (
                <li className="text_des" key={index}>
                  {tr(listItem.label)}
                </li>
              )
            })}
      ...
      {type === 'multi' && (
        <div className="mt-2.5">
          <Upload
            ...
            multiple={false}
          >
            <Button className="primary_btn mt-2.5 flex h-8 items-center gap-x-2">
              <span>+</span>
              {tr('config_file_name')}
            </Button>
          </Upload>
          {confiles &&
            Object.keys(confiles)?.map((acekey: string, index: number) => {
