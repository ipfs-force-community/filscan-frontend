...
export default observer(
  ({ accountId, type }: { accountId?: string | string[]; type: string }) => {
    ...
    const load = async (inter?: string) => {
      ...
      ;(result?.power_trend_by_account_id_list || []).forEach((value: any) => {
      ...
      <div className={classNames(styles['power-change'], 'flex-1')}>
        <div
          className={classNames(
            'mx-2.5 mb-2 flex h-[32px] items-center justify-between',
            styles.title,
          )}
        >
          <div
            className={classNames(styles.between, 'gpa-x-5 flex items-center')}
          >
            <span className={classNames('mr-5 text-lg font-semibold')}>
