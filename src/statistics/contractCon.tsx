/** @format */
import { ContractCntTrend } from '@/contents/apiUrl'
...
export default observer((props: Props) => {
  ...
  const load = async (time?: string) => {
    const seriesObj: any = {}
    contract_con.list.forEach((v) => {
      seriesObj[v.dataIndex] = []
    })
    const dateList: any = []
    const seriesData: any = []
    const inter = time || interval
    const result: any = await axiosData(ContractCntTrend, { interval: inter })
    result?.items?.forEach((value: any) => {
      const {
        block_time,
        contract_counts, //合约地址交易
      } = value
      let showTime =
        inter === '24h'
      ...
      //amount
      seriesObj.contract_counts.push({
        amount: contract_counts,
        value: contract_counts,
        showTime: formatDateTime(block_time, 'YYYY-MM-DD HH:mm'),
        unit: '',
      })
    })

    contract_con.list.forEach((item: any) => {
      seriesData.push({
        type: item.type,
    ...
    <div className={classNames(styles.trend, `h-[full] w-full  ${className}`)}>
      ...
      >
        <div className="flex flex-1 flex-row flex-wrap items-center">
          <div
            className={classNames(
              'font-HarmonyOS w-fit min-w-[120px] pl-2.5 text-lg font-semibold',
              styles.title,
            )}
          >
            {tr('contract_con')}
