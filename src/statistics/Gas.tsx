 ...
 const color = useMemo(() => {
      ...
      yAxis: [
        {
          ...
          axisLabel: {
            formatter(v: any) {     
              return new BigNumber(Number(v)).dividedBy(Math.pow(10, 9)).toFixed(2);
            },
            textStyle: {
  ...
  return (
    <div className="w-full h-full">
      <div className={`flex justify-between text-xs font-PingFang ${origin === 'home' ? 'p-5' : ''} `}>
        {tr(gas.title)}
        {origin === 'home' && <Link href={ `/statistics/gas/`}>
          <Image className="cursor-pointer" src={go} width={18} height={18} alt='go' />
        </Link>}
      </div>
