...
export function formatFil(
  ...
  if (isAccount) {
    ...
    return len
      ? flag +
          Number(returnValue).toLocaleString('en', {
            maximumFractionDigits: len,
          }) +
          ' ' +
          unit
      : flag + Number(returnValue)
  }
  return len
    ? flag + Number(returnValue).toFixed(len)
    : flag + Number(returnValue)
}
...
export function formatNumberUnit(number: number | string, len = 2) {
  ...
  if (num >= 1e9) {
    return (
      Number(num / 1e9).toLocaleString('en', { maximumFractionDigits: len }) +
      'B'
    )
  }
  if (num >= 1e6) {
    return (
      Number(num / 1e6).toLocaleString('en', { maximumFractionDigits: len }) +
      'M'
    )
  }
...
export const account_link = async (value: string) => {
  ...
  switch (show_type) {
    ...
    case 'storageminer':
      return router.push(`/miner/${value}`, `/miner/${value}`, { scroll: true })
    default:
      return router.push(`/address/${value}`, `/address/${value}`, {
        scroll: true,
      })
  }
}
