  ...
  left: [
    {
      ...
      render: (text: number, record) => {
        if (!text) return null
        const changeText =
          record?.percent_change_24h && Number(record.percent_change_24h)
    ...
    },
    {
      ...
      render: (text: number) => (
        <span className="text_primary">
          {text ? formatFilNum(text, false, false, 2) : text}{' '}
        </span>
      ),
    },
    {
...
export const SEO: any = {
  ...
  en: {
    ...
    keywords:
      'Filecoin官方区块浏览器,Filecoin官方浏览器, Filecoin Explorer,fvm,Filscan,Filecoin, blockchain, crypto, currency,最新区块,FIL,IPFS，FIL,Filecoin区块链查询浏览器,FIL浏览器,Filecoin浏览器,Filecoin区块查询,区块链搜索引擎,区块高度,区块链交易',
  ...
  kr: {
    title: '파일코인 익스플로러',
    description:
      'Filecoin 공식 브라우저 ,Filecoin 공식 블록 탐색기 ,Filecoin 블록 쿼리 ,FIL 브라우저 ,FVM ,IPFS, 블록 높이,블록체인 트랜잭션,블록체인 검색 엔진,최신 블록',
    keywords:
      'Filecoin官方区块浏览器,Filecoin官方浏览器, Filecoin Explorer,fvm,Filscan,Filecoin, blockchain, crypto, currency,最新区块,FIL,IPFS，FIL,Filecoin区块链查询浏览器,FIL浏览器,Filecoin浏览器,Filecoin区块查询,区块链搜索引擎,区块高度,区块链交易',
