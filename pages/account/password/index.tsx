...
export default () => {
  ...
  return (
    <>
      <Banner />
      <div className="main_contain !mb-10  !mt-8  !w-2/5 !min-w-[404px]">
        <div className={`tex_color text-lg`}>{tr('forgot_password')}</div>
        <Form
          form={form}
          size="large"
          className="custom_form !mt-7 !flex !w-full !flex-col gap-y-5"
              ...
              if (item.name === 'email') {
                newRules.push(() => ({
                  async validator(_: any, value: any) {
                    const result: any = await axiosData(proApi.mail_exists, {
                      mail: value,
                    })
                    if (!result?.exists) {
