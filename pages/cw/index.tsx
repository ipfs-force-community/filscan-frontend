...
export default observer(() => {
  ...
  const drawChart = (
    data: Array<any>,
    bhm: Record<string, Array<any>> = {},
    blockHeightList: Array<number>,
  ) => {
    ...
    chartRef.current.setOptions({
      ...
      tooltip: {
        ...
        formatter: (toolTipData: any) => {
          ...
          return `
                <div class="tt">
                  <div style="margin-bottom: 10px;">cid: ${
                    toolTipData._id
                  }</div>
                  <div style="margin-bottom: 10px;">miner: ${
                    toolTipData.Miner
                  }</div>
                  <div style="margin-bottom: 10px;">height: ${
                    toolTipData.Epoch
                  }</div>
          ...
          draw: (chart: any, layer: any, s: any) => {
            ...
            yTicks.each(function (yData: any) {
              ...
              t.safeSelect('text').attrs({
                fill:
                  Number(searchHeight.current) === Number(yData)
                    ? 'rgba(29, 107, 253, 1)'
                    : yAxisColor, //y轴字体颜色
                'font-size': 14,
              })
                    ...
                    if (blockGroup) {
                      ...
                      bgEle
                        ...
                        .each(function (d: any, i: number) {
                          ...
                          let showColor =
                            colorsText[numIndex % colorsText.length]
                          if (theme === 'light') {
            ...
            //next link
            let [linkData, forkEndLinkData] = getLinkData(
              data,
              blockMap.current,
            )
            linkGroup
            ...
            if (transformY.current === 0) {
              ...
              axisYWrap
                ...
                .attr(
                  'transform',
                  `translate(0, ${transformY.current + 50}) scale(1,${
                    k.current
                  })`,
                )
            }
            ...
            function zoomed() {
              ...
              stage.attr(
                'transform',
                `translate(${transformX.current + dx}, ${
                  transformY.current + dy
                }) scale(${t.k})`,
              )
              // axisXWrap
              //   .select("g.axis-x")
  ...
  return (
    <div className={`main_contain ${styles['cw_contain']} `}>
      ...
      <div
        className={`${styles['block-header-chart']} card_shadow border_color border `}
        ref={chartContainerRef}
      ></div>
      {!chartLoading && hasData && (
        <div className={`${styles['block-header-chart-noData']}`}>No Data</div>
      )}
      {chartLoading && (
        <div
